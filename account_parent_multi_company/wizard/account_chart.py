...
class OpenAccountChartMultiCompany(models.TransientModel):
    ...
    target_currency_id = fields.Many2one('res.currency', 'Target Currency')
    company_ids = fields.Many2many('res.company', 'res_company_coachart_rel', 'coa_id', 'cid', string='Companies')
    ...
    def onchange_consolidated_coa(self):
        ...
        if self.consolidated_coa:
            if len(allowed_company_ids) <= 1:
                raise exceptions.UserError("Please Allow multiple companies in the multi company options(top right corner)")
    ...
    #     allowed_company_ids  = self.env.context.get('allowed_company_ids', [])
        ...
        # companies = self.env.user.company_ids
        # if self.company_id != companies:
        #     self.view_consolidation = True
        # self.report_type = 'account_type'
        # if len(companies.mapped('country_id.id')) == 1:
        #     self.report_type = 'account'
        
    def _build_contexts(self):
        res = super(OpenAccountChartMultiCompany, self)._build_contexts()
        self.ensure_one()
        if self.consolidated_coa:
    ...
    def get_accounts(self, line_id, context):
        if not context.get('consolidated_coa', False):
            return super(OpenAccountChartMultiCompany, self).get_accounts(line_id=line_id, context=context)
    ...
    def get_at_accounts(self, at_data, context):
        if not context.get('consolidated_coa', False):
            return super(OpenAccountChartMultiCompany, self).get_at_accounts(at_data=at_data, context=context)
    ...
    def get_heading(self, context):
        if not context.get('consolidated_coa', False):
            return super(OpenAccountChartMultiCompany, self).get_heading(context=context)
        companies = self.env['res.company'].browse(context.get('company_ids'))
        res = "Consolidated Chart of Account: %s" % ', '.join(companies.mapped('display_name'))
        return res
    ...
    def _amount_to_str(self, value, currency):
        ...
        if not context.get('consolidated_coa', False):
            return super(OpenAccountChartMultiCompany, self)._amount_to_str(value=value, currency=currency)
        ...
        # print("field_options",field_options,target_currency_id)
    ...
    def _lines(self, wiz_id=None, line_id=None, level=1, obj_ids=[]):
        context = self.env.context
        if not context.get('consolidated_coa', False):
            return super(OpenAccountChartMultiCompany, self)._lines(wiz_id=wiz_id, line_id=line_id, 
                                                                    level=level, obj_ids=obj_ids)
        final_vals = []
        account_code_dict = {}
        ...
        for account in obj_ids:
            ...
            if (account.code, company_id) not in account_code_dict:
                account_code_dict[(account.code, company_id)] = self.line_data(level, wiz_id=wiz_id,
                                                                               parent_id=line_id, account=account)
                if company_id:
                    account_code_dict[(account.code, company_id)]['name'] = "%s (%s)" % (account.name,
                ...
                account_code_dict[(account.code, company_id)]['ini_bal'] += account.initial_balance
                account_code_dict[(account.code, company_id)]['end_bal'] += account.balance + account.initial_balance
                
        for acd in account_code_dict:
            company = account_code_dict[acd]['company_obj']
            account_code_dict[acd]['debit'] = self._float_html_formating(account_code_dict[acd]['db'], company)
            account_code_dict[acd]['credit'] = self._float_html_formating(account_code_dict[acd]['cr'], company)
            account_code_dict[acd]['balance'] = self._float_html_formating(account_code_dict[acd]['bal'], company)
            account_code_dict[acd]['initial_balance'] = self._float_html_formating(account_code_dict[acd]['ini_bal'],
                                                                                   company)
            account_code_dict[acd]['ending_balance'] = self._float_html_formating(account_code_dict[acd]['end_bal'],
                                                                                  company)
